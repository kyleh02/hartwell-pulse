-- =============================================================================
-- Hartwell Pulse — 0002 Row Level Security
-- Run after 0001_schema.sql. This is the real data-isolation boundary: a client
-- can only ever touch rows for their own client_id; Kyle (admin) sees everything.
--
-- Assumes the Supabase <-> Clerk native integration, so the request JWT carries
-- the Clerk user id in `sub` and the Postgres role `authenticated`.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Identity helpers
-- is_admin() and current_client_id() are SECURITY DEFINER so they read
-- client_users without tripping its own RLS (no recursion).
-- -----------------------------------------------------------------------------
...
-- conversation read state will be its own table in the messaging phase, so we
-- never need clients to UPDATE the messages table itself.
...
-- A recipient may mark a notification read, but not rewrite its content (title,
-- body, link, type). This trigger freezes every column except read_at for a
-- signed-in non-admin. Service role (no JWT sub) and admin are unaffected, so
-- server code and Kyle can still write freely.
